...
// function prototypes
vec3 CalcPointLight(Material material, PointLight light, vec3 N, vec3 fragPos, vec3 V, int index);
...
vec3 CalcPointLight(Material material, PointLight light, vec3 N, vec3 fragPos, vec3 V, int index)
{
    ...
    //float attenuation = 1.0; // Disable attenuation
    ...
    vec3 ambient = K_A * light.ambient* flickerFactor;
    vec3 diffuse = K_D * max(dot(N, L), 0.0) * light.diffuse* flickerFactor;
    vec3 specular = K_S * pow(max(dot(V, R), 0.0), material.shininess) * light.specular* flickerFactor;
